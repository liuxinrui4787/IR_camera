...
void QtMaterialFlatButton::setFontBold(bool bold)
{
    Q_D(QtMaterialFlatButton);
...
bool QtMaterialFlatButton::fontBold() const
{
    Q_D(const QtMaterialFlatButton);
...
void QtMaterialFlatButton::paintBackground(QPainter *painter)
{
    ...
    if (Qt::TransparentMode == d->bgMode)
        return;
...
void QtMaterialFlatButton::paintForeground(QPainter *painter)
{
    ...
    if (isEnabled())
    {
        if (d->stateMachine->overlayOpacity() > 0 && Qt::TransparentMode == d->bgMode)
        {
    ...
    if (Material::TopIcon == d->iconPlacement)
    {
        iconGeometry.moveTo((size().width() - iconSize().width()) / 2, (size().height() - iconSize().height()) / 2 - textGeometry.height() + IconPadding - d->iconpadding);
        textGeometry.moveTo((size().width() - textSize.width()) / 2, iconGeometry.bottom() + textGeometry.height() / 2 + d->iconpadding);
    }
    else if (Material::LeftIcon == d->iconPlacement)
    {
